...
File: `art/ui_fighter.png`. Stacked on player select under SELECT.
...
5. **Cursor badges.** While picking the first fighter, show **1P** on the highlighted tile. After the first lock, **1P** stays on that tile; **2P** rides the cursor until the second lock. **Arcade:** **1P** only — never **2P**. Tile frames: drawn arcade rectangles (idle grey; blink gold when that tile is the cursor).
...
7. **Arcade type, not chrome buttons.** Menu labels use the yellow Blitz arcade font + chevrons. Cursor badges stay `ui_1p.png` / `ui_2p.png`.
